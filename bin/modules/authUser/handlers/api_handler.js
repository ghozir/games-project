...
const injectAdmin = async (req, res) => {
  ...
  const validatePayload = validator.isValidPayload(
    payload,
    commandModel.injectAdmin
  );

  const postRequest = (result) =>
    result.err ? result : commandHandler.InjectAdmin(result.data);
...
const login = async (req, res) => {
  ...
  const postRequest = (result) =>
    result.err ? result : commandHandler.login(result.data);
...
const logout = async (req, res) => {
  ...
  const sendResponse = (result) => {
    result.err
      ? wrapper.response(res, 'fail', result, 'Failed to log admin out')
      : wrapper.response(res, 'success', result, 'Successfully log admin out');
  };
...
const getInfo = async (req, res) => {
  return wrapper.response(
    ...
    'success',
    { data: req.user },
    'Successfully get authentication status'
  );
};
...
const forgetPass = async (req, res) => {
  ...
  const validatePayload = validator.isValidPayload(
    payload,
    commandModel.forgetPass
  );

  const postRequest = (result) =>
    result.err ? result : commandHandler.forgetPass(result.data);
